...
export const ArticleCard: React.FC<ArticleCardProps> = ({ item, onClick, isInQueue, onToggleQueue, featured }) => (
  <FadeInSection>
    <div 
      onClick={onClick}
      className="group cursor-pointer relative py-[var(--space-md)] border-b border-[var(--border)]"
    >
      ...
      {/* 
         Wrapper de conteúdo que se move com Transform, não Padding.
         Isso previne Layout Shifts e melhora performance.
      */}
      <div className="transition-transform duration-500 lg:group-hover:translate-x-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                aria-label={isInQueue ? "Remover da lista" : "Adicionar à lista"}
              >
            ...
            <div className="max-w-4xl space-y-2">
              <h3 className={`${featured ? 'type-h1' : 'type-h2'} font-serif font-light leading-tight text-[var(--text-main)] transition-colors duration-500 group-hover:text-[var(--text-muted)]`}>
                {item.question}
              </h3>
              
              <p className={`text-[var(--text-muted)] font-light leading-relaxed line-clamp-2 ${featured ? 'type-body' : 'type-small'} max-w-3xl`}>
                {item.answer}
              </p>
            </div>
