...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const getBtnClass = (isActive: boolean) => `
    flex items-center justify-between w-full type-small py-3 px-3 rounded-lg transition-all duration-300
    ${isActive 
  ...
  const getHeadingClass = (isVisible: boolean) => `
    type-tiny uppercase tracking-[0.25em] text-[var(--text-main)] font-black mb-4 px-3 transition-opacity duration-500
  ...
  return (
    <>
      ...
      <aside 
        ...
        className={`
          fixed left-0 top-0 bottom-0 glass bg-[var(--bg-island)] border-r border-[var(--border)] z-[70] 
          flex flex-col py-8 transition-all duration-500 cubic-bezier(0.25, 1, 0.5, 1)
          ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} 
          ${isExpanded ? 'w-80 px-5' : 'w-24 px-4'}
        `}
      >
        <div className="mb-10 flex items-center justify-between px-3 overflow-hidden min-h-[32px]">
          <div 
            onClick={onLogoClick}
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className={`flex items-center gap-2 transition-opacity duration-500 whitespace-nowrap ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
                    ...
                    {queueCount > 0 && (
                      <span className="type-tiny font-black bg-[var(--text-main)] text-[var(--bg-main)] px-2 py-0.5 rounded-full ml-1">
                        {queueCount}
                      </span>
                    )}
                  </div>
        ...
        <div className="pt-6 border-t border-[var(--border)] px-3">
          <button 
            onClick={toggleDark}
            className="w-full flex items-center justify-between py-2 type-tiny font-black uppercase tracking-widest text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors"
          >
            <div className="flex items-center gap-4">
